...
import { useDeleteAdminJobMutation } from '@/api/store';
import { routes } from '@/config/routes';
import { DELETE_LABELS } from './config/constants';
import styles from './DeleteJobButton.styles';

export default function DeleteJobButton({ jobId }: { jobId: string }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [deleteJob, { isLoading }] = useDeleteAdminJobMutation();
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setOpen(false);
      router.push(routes.jobs.explorer);
    } catch {
