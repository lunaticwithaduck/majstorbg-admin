...
import styles from './JobCategoriesTable.styles';
...
function InlineEditCell({ row }: InlineEditCellProps) {
  ...
  const [update, { isLoading: isUpdating }] = useUpdateAdminJobCategoryMutation();
  const [deleteCategory, { isLoading: isDeleting }] = useDeleteAdminJobCategoryMutation();
...
function NewCategoryForm({ onAdded }: NewCategoryFormProps) {
  ...
  const [create, { isLoading }] = useCreateAdminJobCategoryMutation();
...
export default function JobCategoriesTable() {
  ...
  const { data, isLoading, isFetching, isError } = useListAdminJobCategoriesQuery(queryArgs);
